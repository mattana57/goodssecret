...
/* ================= [ส่วนที่ปรับแก้ไข]: เปลี่ยนที่เก็บสลิปเป็น slips/ ชั้นนอกสุด ================= */
$slip_name = "";
if ($payment_method === 'bank' && isset($_FILES['slip_image']) && $_FILES['slip_image']['error'] == 0) {
    ...
    // เปลี่ยนจาก "uploads/slips/" เป็น "slips/" ตามที่คุณต้องการ
    $target_dir = "slips/";
    
    // ตรวจสอบและสร้างโฟลเดอร์ slips/ หากยังไม่มี
...
try {
    ...
    if ($conn->query($sql_order)) {
        ...
        while ($item = $cart_items->fetch_assoc()) {
            // ใส่ variant_id ลงในการ INSERT เพื่อให้เก็บข้อมูลรุ่นสินค้า
        ...
        // ส่งค่าไปเปิด Modal ที่หน้า profile
