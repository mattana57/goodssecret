...
// --- [Logic 2]: ลูกค้ายกเลิกออเดอร์เอง (เฉพาะสถานะยังไม่ส่ง) ---
if (isset($_POST['customer_cancel'])) {
    $reason = $conn->real_escape_string($_POST['cancel_reason'] ?? 'ลูกค้ายกเลิกเองผ่านหน้าเว็บ');
    // เงื่อนไขคือต้องเป็นสถานะ pending หรือ processing เท่านั้นถึงจะยกเลิกได้เอง
...
// --- [Logic 3]: แนบสลิป/แก้ไขที่อยู่ (คงเดิม) ---
...
<div class="container py-5">
    <div class="glass-card mx-auto" style="max-width: 900px;">
        <div class="d-flex justify-content-between align-items-center mb-5">
            ...
            <h3 class="text-neon-cyan mb-0 FW-BOLD">รายละเอียดบิล #<?= str_pad($order['id'], 5, '0', STR_PAD_LEFT) ?></h3>
        ...
        <?php if($order['status'] == 'cancelled'): ?>
            <div class="alert custom-alert-danger text-center mb-4">
                <h5 class="fw-bold mb-1"><i class="bi bi-x-circle-fill me-2"></i> คำสั่งซื้อนี้ถูกยกเลิกแล้ว</h5>
                <p class="small mb-0 opacity-75">เหตุผล: <?= htmlspecialchars($order['cancel_reason'] ?: 'ไม่ได้ระบุเหตุผล') ?></p>
            </div>
        <?php endif; ?>
            ...
            <tbody>
                ...
                <tr class="align-middle border-bottom border-white border-opacity-10">
                    <td class="d-flex align-items-center">
                        ...
                        <span><?= htmlspecialchars($item['p_name']) ?></span>
                    </td>
        ...
        <div class="mt-4 pt-2">
            <?php if($order['status'] == 'shipped'): ?>
                <div class="p-4 custom-alert-neon text-center shadow-lg">
                    <h5 class="fw-bold mb-3"><i class="bi bi-box2-heart"></i> ได้รับสินค้าเรียบร้อยแล้วใช่ไหม?</h5>
            ...
            <?php elseif(in_array($order['status'], ['pending', 'processing'])): ?>
                <button type="button" class="btn btn-outline-danger w-100 rounded-pill py-3 fw-bold" data-bs-toggle="modal" data-bs-target="#cancelModal">ยกเลิกคำสั่งซื้อนี้</button>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<div class="modal fade" id="cancelModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <form class="modal-content text-center py-4" method="POST">
            <div class="modal-body">
                <i class="bi bi-exclamation-octagon text-danger display-1 mb-4 d-block"></i>
                <h3 class="fw-bold mb-3 text-neon-pink">ต้องการยกเลิกใช่ไหม?</h3>
                <p class="opacity-75 mb-4 px-3">หากกดยกเลิก รายการนี้จะไม่สามารถกู้คืนได้</p>
                <input type="text" name="cancel_reason" class="form-control mb-4" placeholder="ระบุเหตุผล (ถ้ามี)...">
