...
// --- [ส่วน AJAX]: จัดการสต็อก และดึงข้อมูลต่างๆ แบบไม่รีโหลดหน้า ---
if (isset($_GET['ajax_action'])) {
    // ปรับสต็อกสินค้า (เพิ่ม/ลด)
...
<head>
    ...
    <style>
        ...
        .stock-val { min-width: 30px; text-align: center; font-weight: bold; }
    </style>
</head>
<body>
...
<div class="container-fluid py-5 px-4">
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="products">
            <div class="glass-panel">
                ...
                <table class="table table-hover datatable-js">
                    <thead><tr><th>รูป</th><th>ชื่อ</th><th>ราคา</th><th>สต็อกแยกตามรุ่น (ปรับที่นี่)</th><th>จัดการ</th></tr></thead>
                    <tbody>
                            ...
                            <td>
                                ...
                                <div class="mb-1 small d-flex justify-content-between align-items-center border-bottom border-secondary pb-1">
                                    ...
                                    <div class="stock-control">
                                        <button class="btn btn-sm btn-outline-danger py-0 px-2" onclick="updateStock(<?= $v['id'] ?>, -1)">-</button>
                                        <span class="stock-val text-neon-cyan" id="stock_val_<?= $v['id'] ?>"><?= $v['stock'] ?></span>
                                        <button class="btn btn-sm btn-outline-success py-0 px-2" onclick="updateStock(<?= $v['id'] ?>, 1)">+</button>
                                    </div>
                                </div>
                                <?php endwhile; ?>
                            </td>
        ...
        <div class="tab-pane fade" id="categories">
            <div class="glass-panel">
                <div class="d-flex justify-content-between mb-4 align-items-center"><h4>ข้อมูลประเภทสินค้า</h4><button class="btn btn-neon-pink btn-sm rounded-pill px-3" onclick="openCatModal()">+ เพิ่มประเภท</button></div>
                <table class="table table-hover datatable-js">
                    <thead><tr><th>ID</th><th>ชื่อประเภท</th><th>จัดการ</th></tr></thead>
                    <tbody>
                        <?php while($c = $categories_list->fetch_assoc()): ?>
                        <tr><td>#<?= $c['id'] ?></td><td class="fw-bold"><?= $c['name'] ?></td><td><button class="btn btn-sm btn-outline-warning" onclick='openEditCat(<?= json_encode($c) ?>)'><i class="bi bi-pencil"></i></button> <button class="btn btn-sm btn-outline-danger" onclick="confirmAction('category', <?= $c['id'] ?>, 'categories')"><i class="bi bi-trash"></i></button></td></tr>
                        <?php endwhile; ?>
                    </tbody>
                </table>
            </div>
        ...
        <div class="tab-pane fade" id="customers">
            <div class="glass-panel">
                <div class="d-flex justify-content-between mb-4 align-items-center"><h4>จัดการข้อมูลลูกค้า</h4><button class="btn btn-neon-pink rounded-pill px-4" onclick="openAddUser()">+ เพิ่มลูกค้า</button></div>
                <table class="table table-hover datatable-js">
            ...
            <div class="row g-4">
                ...
                <div class="col-md-5">
                    ...
                    <form method="POST"><input type="hidden" name="order_id" id="v_input_oid">
                        <select name="status" id="v_select_status" class="form-select mb-3"><option value="pending">รอตรวจสอบ</option><option value="paid">ชำระเงินแล้ว</option><option value="shipped">จัดส่งแล้ว</option><option value="cancelled">ยกเลิก</option></select>
                        <button type="submit" name="update_order_status" class="btn btn-success w-100">บันทึก</button>
                    </form>
                </div>
            </div>
        </div>
...
<div class="modal fade" id="userModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST">
        ...
        <div class="modal-footer border-secondary"><button type="submit" name="save_user" class="btn btn-primary w-100">บันทึกข้อมูล</button></div>
    </form></div>
</div>

<div class="modal fade" id="confirmDeleteModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered"><div class="modal-content text-center py-4"><div class="modal-body">
        <i class="bi bi-exclamation-triangle text-warning display-4 mb-3"></i><h4 class="mb-3">ยืนยันการลบ?</h4>
        <div class="d-flex gap-2 justify-content-center"><button type="button" class="btn btn-outline-light px-4 rounded-pill" data-bs-dismiss="modal">ยกเลิก</button><a href="#" id="confirmDeleteBtn" class="btn btn-danger px-4 rounded-pill text-decoration-none">ลบทันที</a></div>
    </div></div></div>
</div>
...
<div class="modal fade" id="productModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            ...
            <div class="row"><div class="col-6 mb-3"><label>ราคา</label><input type="number" name="price" id="p_price" class="form-control" required></div><div class="col-6 mb-3"><label>ประเภท</label><select name="category_id" id="p_cat" class="form-select"><?php $categories_list->data_seek(0); while($c=$categories_list->fetch_assoc()): ?><option value="<?=$c['id']?>"><?=$c['name']?></option><?php endwhile; ?></select></div></div>
            <div class="mb-3"><label>รายละเอียด</label><textarea name="description" id="p_desc" class="form-control" rows="2"></textarea></div><div class="mb-3"><label>รูปภาพ</label><input type="file" name="image" class="form-control"></div>
        </div>
...
<div class="modal fade" id="variantModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST" enctype="multipart/form-data">
        <div class="modal-header border-secondary"><h5 class="modal-title">เพิ่มรุ่น: <span id="v_pname"></span></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
        <div class="modal-body"><input type="hidden" name="product_id" id="v_pid"><div class="mb-3"><label>ชื่อรุ่น</label><input type="text" name="v_name" class="form-control" required></div><div class="mb-3"><label>สต็อก</label><input type="number" name="v_stock" class="form-control" value="10"></div><div class="mb-3"><label>รูปภาพรุ่น</label><input type="file" name="v_image" class="form-control" required></div></div>
...
<div class="modal fade" id="catModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST"><div class="modal-header border-secondary"><h5 class="modal-title" id="cat_title">จัดการประเภท</h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div><div class="modal-body"><input type="hidden" name="cat_id" id="cat_id"><div class="mb-3"><label>ชื่อประเภท</label><input type="text" name="cat_name" id="cat_name" class="form-control" required></div></div><div class="modal-footer border-secondary"><button type="submit" name="save_category" class="btn btn-primary">บันทึก</button></div></form></div>
</div>
...
<script>
    ...
    function updateStock(vid, amount) { $.get('admin_dashboard.php', {ajax_action: 'update_stock', vid: vid, amount: amount}, function(new_stock) { $(`#stock_val_${vid}`).text(new_stock).fadeOut(100).fadeIn(100); }); }
    function openOrderView(o) { $('#v_oid').text(o.id); $('#v_input_oid').val(o.id); $('#v_select_status').val(o.status); $('#v_addr').html(`<strong>ผู้รับ:</strong> ${o.fullname}<br><strong>ที่อยู่:</strong> ${o.address}`); $('#v_total').text(`ยอดสุทธิ: ฿${new Intl.NumberFormat().format(o.total_price)}`); $.get('admin_dashboard.php', {ajax_action: 'get_order_items', oid: o.id}, function(data) { $('#v_items').html(data); }); $('#orderViewModal').modal('show'); }
    function viewUserHistory(uid, uname) { $('#h_uname').text(uname); $('#h_content').html('<p class="text-center py-4 opacity-50">กำลังดึงประวัติ...</p>'); $('#historyModal').modal('show'); $.get('admin_dashboard.php', {ajax_action: 'get_user_history', uid: uid}, function(data) { $('#h_content').html(data); }); }
    function openAddUser() { $('#u_id').val(''); $('#u_uname').val(''); $('#u_email').val(''); $('#u_fname').val(''); $('#u_phone').val(''); $('#u_addr').val(''); $('#userModal').modal('show'); }
    ...
    function openAddProduct() { $('#p_id').val(''); $('#productModal').modal('show'); }
