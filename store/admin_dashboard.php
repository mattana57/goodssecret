    ...
    if ($p_id) {
        ...
        if($is_variant == 'yes' && isset($_POST['v_names'])) {
            foreach($_POST['v_names'] as $i => $vname) {
                ...
                $conn->query("INSERT INTO product_variants (product_id, variant_name, price, stock) VALUES ($new_p_id, '".$conn->real_escape_string($vname)."', '$vprice', $vstock)");
            }
        }
    }
...
<div class="container-fluid py-5 px-4">
    ...
    <div class="tab-content">
        ...
        <div class="tab-pane fade" id="categories">
            <div class="glass-panel">
                <div class="d-flex justify-content-between mb-4 align-items-center"><h4>จัดการประเภท</h4><button class="btn btn-neon-pink btn-sm rounded-pill" onclick="openCatModal()">+ เพิ่มประเภท</button></div>
                <table class="table table-hover datatable-js w-100">
        ...
        <div class="tab-pane fade" id="customers">
            <div class="glass-panel">
                ...
                <table class="table table-hover datatable-js w-100">
                    ...
                    <tbody><?php while($u = $users_list->fetch_assoc()): ?><tr><td><?= $u['username'] ?></td><td><?= $u['fullname'] ?></td><td><?= $u['phone'] ?></td><td><button class="btn btn-sm btn-outline-warning" onclick='openEditUser(<?= json_encode($u) ?>)'><i class="bi bi-pencil"></i></button> <button class="btn btn-sm btn-outline-danger" onclick="confirmAction('user', <?= $u['id'] ?>, 'customers')"><i class="bi bi-trash"></i></button></td></tr><?php endwhile; ?></tbody>
                </table>
            </div>
        </div>
        ...
        <div class="tab-pane fade" id="orders">
            <div class="glass-panel">
                ...
                <table class="table table-hover datatable-js w-100">
                    ...
                    <tbody><?php while($o = $orders_list->fetch_assoc()): ?><tr><td><?= date('d/m/y H:i', strtotime($o['created_at'])) ?></td><td>#<?= str_pad($o['id'], 5, '0', STR_PAD_LEFT) ?></td><td><?= $o['fullname'] ?></td><td>฿<?= number_format($o['total_price']) ?></td><td><span class="badge bg-secondary"><?= strtoupper($o['status']) ?></span></td><td><a href="admin_order_view.php?id=<?= $o['id'] ?>" class="btn btn-sm btn-outline-info rounded-pill"><i class="bi bi-receipt"></i> จัดการบิล</a></td></tr><?php endwhile; ?></tbody>
                </table>
            </div>
        </div>
    </div>
    ...
    <div class="modal-dialog modal-lg"><form class="modal-content" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            ...
            <div class="row g-3">
                ...
                <div id="variant_fields" style="display:none;" class="col-12"><div id="variant_container"></div><button type="button" class="btn btn-sm btn-outline-info" onclick="addVariantRow()">+ เพิ่มรุ่นย่อย</button></div>
            </div>
        </div>
...
<div class="modal fade" id="catModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST">
        ...
        <div class="modal-footer border-secondary"><button type="submit" name="save_category" class="btn btn-neon-pink w-100">บันทึกข้อมูล</button></div>
    </form></div>
</div>
...
<script>
    ...
    function openAddProduct() { $('#productModal').find('form')[0].reset(); $('#p_id').val(''); toggleVariantFields('no'); $('#productModal').modal('show'); }
