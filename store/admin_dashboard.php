...
if (isset($_POST['save_product'])) {
    ...
    if (!empty($_POST['product_id'])) {
        ...
        if($is_variant == 'no') { $conn->query("INSERT INTO product_variants (product_id, variant_name, stock) VALUES ($new_p_id, 'สินค้าทั่วไป', 0)"); }
    }
...
<div class="container-fluid py-5 px-4">
    ...
    <div class="tab-content">
        <div class="tab-pane fade show active" id="products">
            <div class="glass-panel">
                ...
                <table class="table table-hover datatable-js">
                    <thead><tr><th>รูป</th><th>ชื่อสินค้า</th><th>ราคา</th><th>จำนวนสต็อก (พิมพ์เลข)</th><th>จัดการ</th></tr></thead>
                    <tbody>
        ...
        <div class="tab-pane fade" id="categories">
            <div class="glass-panel">
                <div class="d-flex justify-content-between mb-4 align-items-center"><h4>ข้อมูลประเภทสินค้า</h4><button class="btn btn-neon-pink btn-sm rounded-pill" onclick="openCatModal()">+ เพิ่มประเภท</button></div>
                <table class="table table-hover datatable-js">
                ...
                <table class="table table-hover datatable-js">
                    ...
                    <tbody>
                        ...
                        <tr class="align-middle">
                            ...
                            <td><?= $u['fullname'] ?: '-' ?></td>
                            <td><?= $u['phone'] ?></td>
                            <td style="max-width: 250px;"><small class="opacity-75"><?= $u['address'] ?: '-' ?></small></td>
                            <td>
                                <div class="btn-group">
...
<div class="modal fade" id="historyModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <div class="modal-header border-secondary"><h5 class="modal-title">ประวัติ: <span id="h_uname"></span></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
...
<div class="modal fade" id="orderViewModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg"><div class="modal-content">
        <div class="modal-header border-secondary"><h5 class="modal-title">บิล #<span id="v_oid"></span></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
        <div class="modal-body">
            <div class="row g-4">
                <div class="col-md-7 border-end border-secondary">
                    <h6 class="text-info fw-bold">ที่อยู่จัดส่ง</h6><p id="v_addr" class="small text-white"></p>
                    <hr class="border-secondary"><h6 class="text-info fw-bold">รายการสินค้า</h6><div id="v_items"></div>
                    <h4 class="text-neon-pink mt-3" id="v_total"></h4>
                </div>
                <div class="col-md-5">
                    <h6 class="text-info fw-bold">ปรับสถานะ</h6>
...
<div class="modal fade" id="productModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST" enctype="multipart/form-data">
        ...
        <div class="modal-body">
            ...
            <div class="mb-3"><label>มีรุ่นย่อยไหม?</label><select name="is_variant" class="form-select border-info"><option value="no">ไม่มี (สินค้าทั่วไป)</option><option value="yes">มี (แบบเลือกตัวเลือก)</option></select></div>
            ...
            <div class="mb-3"><label>รายละเอียด</label><textarea name="description" id="p_desc" class="form-control" rows="2"></textarea></div><div class="mb-3"><label>รูปหลัก</label><input type="file" name="image" class="form-control"></div>
        </div>
        <div class="modal-footer border-secondary"><button type="submit" name="save_product" class="btn btn-neon-pink w-100">บันทึกสินค้า</button></div>
    </form></div>
</div>
...
<div class="modal fade" id="userModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST">
        ...
        <div class="modal-footer border-secondary"><button type="submit" name="save_user" class="btn btn-neon-pink w-100">บันทึก</button></div>
    </form></div>
</div>

<div class="modal fade" id="variantModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog"><form class="modal-content" method="POST" enctype="multipart/form-data">
        <div class="modal-header border-secondary"><h5>เพิ่มรุ่น: <span id="v_pname"></span></h5><button type="button" class="btn-close btn-close-white" data-bs-dismiss="modal"></button></div>
        <div class="modal-body"><input type="hidden" name="product_id" id="v_pid"><div class="mb-3"><label>ชื่อรุ่น</label><input type="text" name="v_name" class="form-control" required></div><div class="mb-3"><label>สต็อกเริ่มต้น</label><input type="number" name="v_stock" class="form-control" value="0"></div><div class="mb-3"><label>รูปภาพรุ่น</label><input type="file" name="v_image" class="form-control"></div></div>
        <div class="modal-footer border-secondary"><button type="submit" name="add_variant" class="btn btn-primary">บันทึก</button></div>
    </form></div>
</div>
...
<script>
    ...
    function manualUpdateStock(vid, new_val) { $.get('admin_dashboard.php', {ajax_action: 'update_stock_value', vid: vid, val: new_val}, function(data) { console.log("Updated: " + data); }); }
