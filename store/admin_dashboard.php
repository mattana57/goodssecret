...
if (isset($_GET['ajax_action'])) {
    ...
    if ($_GET['ajax_action'] == 'get_user_history') {
        ...
        while($r = $q->fetch_assoc()) {
            ...
            $html .= '<tr class="align-middle text-white"><td>#'.str_pad($r['id'], 5, '0', STR_PAD_LEFT).'</td><td>'.date('d/m/y H:i', strtotime($r['created_at'])).'</td><td class="fw-bold">฿'.number_format($r['total_price']).'</td><td><span class="badge '.$status_class.'">'.$r['status'].'</span></td><td><a href="admin_order_view.php?id='.$r['id'].'" class="btn btn-sm btn-outline-cyan py-0"><i class="bi bi-search"></i> ดูบิล</a></td></tr>';
        }
...
// --- [Logic จัดการสินค้า]: ระบบอัปโหลดรูปภาพและบันทึกข้อมูล ---
...
// --- Logic จัดการหมวดหมู่, ลูกค้า และการลบ ---
    ...
    <div class="tab-content">
                ...
                <table class="table table-hover datatable-js w-100">
                    ...
                    <tbody>
                        ...
                        <tr class="align-middle">
                            ...
                            <td>
                                <?php if (!$has_variants): ?>
                                <input type="number" class="form-control form-control-sm stock-input mx-auto" value="<?= $p['stock'] ?>" onchange="manualUpdateStockDirect(<?= $p['id'] ?>, this.value)">
                                <?php else: ?>
                ...
                <table class="table table-hover datatable-js w-100">
                    ...
                    <tbody>
                        ...
                        <tr class="align-middle">
                            ...
                            <td>
                                <a href="admin_order_view.php?id=<?= $o['id'] ?>" class="btn btn-sm btn-outline-info rounded-pill px-3">
                                    <i class="bi bi-receipt"></i> จัดการบิล
                                </a>
                            </td>
                        </tr>
                        <?php endwhile; ?>
        ...
        <div class="tab-pane fade" id="customers">
            <div class="glass-panel">
                ...
                <table class="table table-hover datatable-js w-100">
                    <thead><tr><th>User / Email</th><th>ชื่อ-นามสกุล</th><th>เบอร์โทร</th><th>จัดการ</th></tr></thead>
                    <tbody><?php while($u = $users_list->fetch_assoc()): ?><tr><td><?= $u['username'] ?><br><small class="text-neon-cyan"><?= $u['email'] ?></small></td><td><?= $u['fullname'] ?></td><td><?= $u['phone'] ?></td><td><button class="btn btn-sm btn-outline-warning" onclick='openEditUser(<?= json_encode($u) ?>)'><i class="bi bi-pencil"></i></button> <button class="btn btn-sm btn-outline-danger" onclick="confirmAction('user', <?= $u['id'] ?>, 'customers')"><i class="bi bi-trash"></i></button></td></tr><?php endwhile; ?></tbody>
                </table>
            </div>
        </div>
        ...
        <form class="modal-content" method="POST" enctype="multipart/form-data">
            ...
            <div class="modal-body">
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-3"><label>มีรุ่นย่อยไหม?</label><select name="is_variant" id="is_variant_select" class="form-select border-info" onchange="toggleVariantFields(this.value)"><option value="no">ไม่มีรุ่นย่อย</option><option value="yes">มีรุ่นย่อย</option></select></div>
                    ...
                    <div id="variant_fields" class="col-12 mt-4" style="display: none;">
                        <h6 class="text-neon-cyan mb-2">รายการรุ่นย่อย (ชื่อ, ราคา, สต็อก)</h6>
                        <div id="variant_container"></div>
                        <button type="button" class="btn btn-sm btn-outline-info" onclick="addVariantRow()">+ เพิ่มแถวรุ่นย่อย</button>
                    </div>
                </div>
            </div>
...
<script>
    ...
    function addVariantRow() { $('#variant_container').append(`<div class="variant-card mb-2 p-2 border border-secondary rounded"><div class="row g-2 align-items-end"><div class="col-md-5"><label class="small">ชื่อรุ่น</label><input type="text" name="v_names[]" class="form-control form-control-sm" required></div><div class="col-md-3"><label class="small">ราคา</label><input type="number" name="v_prices[]" class="form-control form-control-sm" required></div><div class="col-md-3"><label class="small">สต็อก</label><input type="number" name="v_stocks[]" class="form-control form-control-sm" value="0"></div><div class="col-md-1"><button type="button" class="btn btn-sm btn-outline-danger" onclick="this.closest('.variant-card').remove()"><i class="bi bi-trash"></i></button></div></div></div>`); }
    ...
    function openAddProduct() { $('#productModal').find('form')[0].reset(); $('#p_id').val(''); toggleVariantFields('no'); $('#p_title').text('เพิ่มสินค้าใหม่'); $('#productModal').modal('show'); }
