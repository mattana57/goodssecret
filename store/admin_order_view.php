        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post(window.location.href, { ajax_status: 1, status: newStatus }, function() {
                    Swal.fire('สำเร็จ!', 'สถานะออเดอร์ถูกเปลี่ยนแล้ว', 'success').then(() => window.location.reload());
                });
            }
        });
    }
        ...
        Swal.fire({
            ...
            if (result.isConfirmed && result.value) {
                $.post(window.location.href, { ajax_status: 1, status: 'cancelled', reason: result.value }, function() {
                    Swal.fire('ยกเลิกแล้ว!', 'ออเดอร์ถูกยกเลิกเรียบร้อย', 'success').then(() => window.location.reload());
                });
            } else if (result.isConfirmed && !result.value) {
                Swal.fire('ผิดพลาด', 'กรุณาระบุเหตุผลการยกเลิก', 'error');
            }
        });
    }
</script>
