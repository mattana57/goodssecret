...
// --- [Logic ใหม่]: อัปเดตสถานะแบบ Real-time (AJAX) เพื่อให้กดได้หลายครั้ง ---
if (isset($_POST['ajax_status'])) {
    $new_status = $conn->real_escape_string($_POST['status']);
    $cancel_reason = $conn->real_escape_string($_POST['reason'] ?? '');
    
    $sql = "UPDATE orders SET status = '$new_status', cancel_reason = '$cancel_reason' WHERE id = $order_id";
    if ($conn->query($sql)) {
    ...
    <div class="row g-4">
        <div class="col-md-4">
            <div class="glass-card mb-4">
                <h5 class="text-info border-bottom border-secondary pb-2 mb-3">ข้อมูลการจัดส่ง</h5>
                <p class="mb-1"><strong>ผู้รับ:</strong> <?= htmlspecialchars($order['fullname']) ?></p>
                <p class="mb-1 small"><strong>อีเมล:</strong> <?= htmlspecialchars($order['email']) ?></p>
                <p class="mb-1 small"><strong>โทร:</strong> <?= htmlspecialchars($order['phone']) ?></p>
                ...
                <p class="mb-1 small"><strong>ไปรษณีย์:</strong> <?= htmlspecialchars($order['zipcode'] ?? '-') ?></p>
            </div>
            
            <div class="glass-card border-warning">
                <h5 class="text-warning border-bottom border-secondary pb-2 mb-3">หลักฐานการโอน</h5>
        ...
        <div class="col-md-8">
            <div class="glass-card mb-4 text-white">
                <h5 class="text-info mb-3">รายการสินค้า</h5>
                <table class="table table-dark table-hover mb-0">
                    ...
                    <tfoot><tr><td colspan="2" class="text-end fw-bold">ยอดรวมสุทธิ:</td><td class="text-end h4 text-neon-pink fw-bold">฿<?= number_format($order['total_price']) ?></td></tr></tfoot>
                </table>
            </div>
            ...
            <div class="glass-card bg-black border-info">
                ...
                <div class="row g-3 align-items-end">
                    <div class="col-md-6">
                        ...
                        <select id="statusSelect" class="form-select bg-dark text-white border-secondary mt-1">
                    ...
                    <div class="col-md-6" id="cancelBox" style="display: <?= ($order['status'] == 'cancelled') ? 'block' : 'none' ?>;">
                        ...
                        <input type="text" id="cancel_reason" class="form-control bg-dark text-white border-danger mt-1" value="<?= htmlspecialchars($order['cancel_reason'] ?? '') ?>" placeholder="แจ้งให้ลูกค้าทราบ...">
                    </div>
                    <div class="col-12 mt-4 text-end">
                        <button type="button" onclick="saveStatusNow()" class="btn btn-success rounded-pill px-5 py-2 fw-bold shadow-lg">บันทึกข้อมูล</button>
                    </div>
                </div>
            </div>
        </div>
...
<script>
    $('#statusSelect').change(function() {
        $('#cancelBox').toggle($(this).val() === 'cancelled');
    });

    function saveStatusNow() {
        const s = $('#statusSelect').val();
        const r = $('#cancel_reason').val();
        const btn = $('button[onclick="saveStatusNow()"]');
        ...
        $.post(window.location.href, {
            ...
            if(res.status === 'success') {
                ...
                setTimeout(() => {
                    btn.html('บันทึกข้อมูล').prop('disabled', false).addClass('btn-success').removeClass('btn-info');
                }, 2000);
            }
        });
    }
