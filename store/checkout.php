                ...
                <div class="card-custom h-100">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <input type="tel" name="phone" class="form-control" value="<?= htmlspecialchars($user_info['phone'] ?? '') ?>" required>
                        </div>
                        <div class="col-12">
                ...
                <div class="card-custom">
                    ...
                    <div class="d-flex justify-content-between mb-3 border-bottom border-secondary border-opacity-25 pb-3">
                        <div class="d-flex flex-column">
                            <span class="fw-bold text-white"><?= htmlspecialchars($item['name']) ?> x <?= $item['quantity'] ?></span>
                            <?php if(!empty($item['variant_name'])): ?>
                                <span class="text-neon-purple small"><i class="bi bi-info-circle me-1"></i>แบบ: <?= htmlspecialchars($item['variant_name']) ?></span>
                            <?php endif; ?>
                        </div>
                        <span class="fw-bold text-neon-pink">฿<?= number_format($item['price'] * $item['quantity']) ?></span>
                    </div>
                    <?php endforeach; ?>
...
<div class="modal fade" id="slipWarningModal" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content custom-popup text-center py-5">
            <div class="modal-body">
                <i class="bi bi-exclamation-circle text-neon-pink display-1 mb-4"></i>
                <h3 class="text-neon-purple fw-bold mb-3">ลืมแนบสลิปหรือเปล่า?</h3>
                <p class="opacity-75 mb-4">คุณเลือกชำระแบบโอนเงิน กรุณาแนบภาพสลิปหลักฐานการโอนเงินเพื่อความรวดเร็วในการตรวจสอบครับ ✨</p>
                <button type="button" class="btn btn-confirm py-2" data-bs-dismiss="modal" style="width: 150px;">เข้าใจแล้ว</button>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    const warningModal = new bootstrap.Modal(document.getElementById('slipWarningModal'));
