...
if (isset($_POST['update_product'])) {
    ...
    // จัดการรุ่นย่อย (ลบของเก่าแล้วเพิ่มใหม่เพื่อให้รองรับการเพิ่ม/ลดแถว)
...
<head>
    ...
    <style>
        ...
        .form-control { background: rgba(255, 255, 255, 0.1) !important; color: #fff !important; border: 1px solid #444 !important; }
        ...
        .btn-remove { position: absolute; top: 10px; right: 10px; color: #ff4d4d; cursor: pointer; font-size: 1.5rem; }
    </style>
</head>
<body>
<div class="container">
    <form method="POST" enctype="multipart/form-data">
        <div class="glass-card mb-4">
            <h3 class="text-info mb-4">📦 ข้อมูลสินค้าหลัก</h3>
            <div class="row g-4">
                ...
                <div class="col-md-8">
                    ...
                    <div class="form-check form-switch mb-3">
                        ...
                        <label class="form-check-label text-info" for="has_variants">เปิดใช้งานรุ่นย่อย (Variants)</label>
                    </div>

                    <div id="main_stats" style="<?= ($variants->num_rows > 0) ? 'display:none;' : '' ?>">
                        <div class="row">
                            <div class="col-6"><label>ราคาหลัก</label><input type="number" name="main_price" class="form-control" value="<?= $p['price'] ?>"></div>
                            <div class="col-6"><label>สต็อกหลัก</label><input type="number" name="main_stock" class="form-control" value="<?= $p['stock'] ?>"></div>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div id="variant_section" class="glass-card" style="<?= ($variants->num_rows > 0) ? '' : 'display:none;' ?>">
            <div class="d-flex justify-content-between align-items-center mb-4">
                <h3 class="text-warning"><i class="bi bi-layers-half me-2"></i>รุ่นย่อย (Variants)</h3>
                <button type="button" class="btn btn-outline-warning rounded-pill" onclick="addVariant()"><i class="bi bi-plus-lg"></i> เพิ่มรุ่น</button>
            </div>
            ...
            <div id="variant_container">
                <?php while($v = $variants->fetch_assoc()): ?>
                    <div class="variant-card">
                        ...
                        <div class="row g-3">
                            <div class="col-md-2">
                                <img src="images/<?= $v['variant_image'] ?>" class="img-fluid rounded mb-2" style="height: 80px; object-fit: cover;">
                            ...
                            <div class="col-md-10">
                                <div class="row g-2">
                                    ...
                                    <div class="col-12"><textarea name="v_desc[]" class="form-control" placeholder="รายละเอียดรุ่น" rows="1"><?= htmlspecialchars($v['variant_description']) ?></textarea></div>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="text-end mt-4">
            <button type="submit" name="update_product" class="btn btn-info rounded-pill px-5 fw-bold">บันทึกทั้งหมด</button>
        </div>
    </form>
</div>
...
function addVariant() {
    ...
    <div class="variant-card">
        <i class="bi bi-x-circle-fill btn-remove" onclick="this.parentElement.remove()"></i>
        <div class="row g-3">
            <div class="col-md-2"><input type="file" name="v_image[]" class="form-control form-control-sm"></div>
            <div class="col-md-10">
                <div class="row g-2">
                    <div class="col-md-6"><input type="text" name="v_name[]" class="form-control" placeholder="ชื่อรุ่น"></div>
