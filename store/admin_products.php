<?php
// --- [Logic 1]: รับค่าเพื่อส่งข้อมูลกลับไปแสดงใน Modal แก้ไข (ธรรมดาๆ เหมือนหน้าลูกค้า) ---
...
// --- [Logic 2]: บันทึกสินค้าใหม่ / อัปเดตสินค้าเดิม ---
...
<style>
    ...
    .text-neon-cyan { color: #00f2fe !important; }
    .btn-edit-neon { border: 1px solid #ffc107 !important; color: #ffc107 !important; background: transparent; }
    .btn-edit-neon:hover { background: #ffc107 !important; color: #000 !important; }
</style>
...
<div class="glass-panel mt-2">
    <div class="d-flex justify-content-between mb-4 align-items-center">
        ...
        <button type="button" class="btn btn-neon-pink rounded-pill px-4 shadow" onclick="openAddModal()">+ เพิ่มสินค้าใหม่</button>
    </div>
    
    <div class="table-responsive">
        <table class="table table-hover w-100">
            <thead><tr class="text-neon-cyan"><th>รูป</th><th>ชื่อสินค้า</th><th>ราคา</th><th>สต็อก</th><th class="text-center">จัดการ</th></tr></thead>
            <tbody>
                ...
                <tr class="align-middle">
                    ...
                    <td class="text-center">
                        <button type="button" class="btn btn-sm btn-edit-neon px-3" onclick="openEditModal(<?= $p['id'] ?>)">
            ...
            <div class="modal-body p-4 text-white">
                ...
                <div class="row g-3">
                    ...
                    <div id="variant_section_box" style="display:none;" class="col-12 mt-4 pt-3 border-top border-secondary">
                        ...
                        <button type="button" class="btn btn-sm btn-outline-info rounded-pill mt-3" onclick="addVariantRowAction()">+ เพิ่มแถว</button>
                    </div>
                </div>
            </div>
            <div class="modal-footer border-secondary px-4 pb-4">
                <button type="submit" id="btn_submit_product" name="save_product" class="btn btn-neon-pink w-100 py-3 fw-bold">บันทึกข้อมูล</button>
            </div>
        </form>
    </div>
</div>
...
function toggleVariantDisplay(val) {
    if (val === 'yes') { $('#variant_section_box').show(); $('#no_variant_row').hide(); } 
    else { $('#variant_section_box').hide(); $('#no_variant_row').show(); }
}

function openAddModal() {
    $('#modal_title').text('เพิ่มสินค้าใหม่');
    $('#btn_submit_product').attr('name', 'save_product').text('บันทึกข้อมูล');
...
// [จุดสำคัญ]: ฟังก์ชันธรรมดาๆ ที่กดแล้วเด้งหน้าแก้ไข
function openEditModal(pid) {
    $('#variant_list_container').empty();
    $.getJSON('admin_products.php', { get_product_json: pid }, function(data) {
        $('#modal_title').text('แก้ไขข้อมูลสินค้า');
        $('#btn_submit_product').attr('name', 'update_product').text('อัปเดตข้อมูล');
        ...
        if (data.variants.length > 0) {
            $('#v_select_field').val('yes'); toggleVariantDisplay('yes');
            data.variants.forEach(function(v) { addVariantRowAction(v); });
        } else {
            $('#v_select_field').val('no'); toggleVariantDisplay('no');
            $('#form_price').val(data.product.price);
            $('#form_stock').val(data.product.stock);
        }
        new bootstrap.Modal(document.getElementById('pModalFull')).show();
    }).fail(function() { alert('ไม่สามารถดึงข้อมูลได้'); });
}
...
function addVariantRowAction(data = null) {
    ...
    const html = `
        <div class="variant-item-row p-3 border border-secondary border-opacity-25 rounded-3 mb-2">
            <input type="hidden" name="v_ids[]" value="${vid}">
            <div class="row g-2 align-items-end">
                <div class="col-md-5"><label class="small">ลาย/รุ่น</label><input type="text" name="v_names[]" class="form-control" value="${vname}" required></div>
                ...
                <div class="col-md-2 text-end"><button type="button" class="btn btn-sm btn-outline-danger" onclick="$(this).closest('.variant-item-row').remove()">ลบ</button></div>
            </div>
        </div>`;
