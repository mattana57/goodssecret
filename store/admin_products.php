<?php
// --- [Logic 1]: รับค่า AJAX เพื่อส่งข้อมูลสินค้ากลับไปแสดงใน Modal (จุดสำคัญที่ทำให้ปุ่มกดได้) ---
if (isset($_GET['get_product_json'])) {
    $id = intval($_GET['get_product_json']);
    // ดึงข้อมูลสินค้าและรุ่นย่อยทั้งหมด
    ...
    exit(); // หยุดการทำงานของ PHP เพื่อส่งแค่ JSON กลับไป
}
...
<style>
    /* สไตล์ความสว่างตัวหนังสือและธีมนีออนตามที่พี่ชอบ */
...
<div class="glass-panel mt-2">
    ...
    <div class="table-responsive">
        <table class="table table-hover w-100 datatable-js">
            <thead>
...
function openEditModal(pid) {
    $('#variant_list_container').empty();
    // ดึงข้อมูลผ่าน AJAX มาใส่ใน Modal โดยเรียกไปที่ตัวมันเอง (admin_products.php)
    $.ajax({
...
function addVariantRowAction(data = null) {
    ...
    const html = `
        <div class="variant-item-row bg-black bg-opacity-25 p-3 border border-secondary border-opacity-50 rounded-3 mb-3 shadow-sm">
            <input type="hidden" name="v_ids[]" value="${vid}">
            <input type="hidden" name="existing_v_images[]" value="${vimg_old}">
            <div class="row g-2">
                <div class="col-md-4"><label class="small text-white-bright mb-1">ลาย/รุ่น</label><input type="text" name="v_names[]" class="form-control" value="${vname}" required></div>
                ...
                <div class="col-md-3"><label class="small text-white-bright mb-1">รูป</label><input type="file" name="v_images[]" class="form-control"></div>
